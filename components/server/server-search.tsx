...
const ServerSearch: React.FC<ServerSearchProps> = ({ data }) => {
  ...
  // on components unmounted
  return () => document.removeEventListener("keydown",down)
   },[])

   // redirect on channel/member from selections 
   const redirectOnClick = ({id, type}:{id:string, type:"channel" |"member"}) =>{
  setOpen(false);
   if(type == "member"){
  ...
  return (
    <>
      ...
      onClick={() =>setOpen(true)}
        className="group px-2 py-2 rounded-md flex items-end
